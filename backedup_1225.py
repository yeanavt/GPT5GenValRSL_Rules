...
class JetBrainsRuleGenerator:
    ...
    def generate_rule(self, row_data: RowData) -> str:
        ...
        7. If the generated RSL rule is too large and these rules are not dependent of each other, 
           split it into multiple independent RSL rules so that each rule examines only one metadata misuse pattern. 
